...
for FOLD_JSONL in "$FOLDS"/fold_*/tuning_set.jsonl; do
  ...
  for SEED in 0 1 2 3 4; do
    python3 benchmark/decomp_generate.py \
      ...
      --config "+adapter LOCO ($TAG)" --data "$FOLD_JSONL" \
      --temperature 0.7 --seed "$SEED" \
      --out "$OUTDIR/predictions/${FOLD}_adapter_${SEED}.jsonl"
    python3 benchmark/decomp_baseline_generate.py \
      --base "$MODEL" --model-revision "$REV" --strategy fewshot --shots 2 \
      --config "base+2shot LOCO ($TAG)" --data "$FOLD_JSONL" \
      --temperature 0.7 --seed "$SEED" \
      --out "$OUTDIR/predictions/${FOLD}_fewshot_${SEED}.jsonl"
  done
done
